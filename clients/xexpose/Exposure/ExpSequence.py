...
class ExpSequence(Actor.Acting):
    ...
    def _finishExposure(self):
        ...
        if self.cntLeft <= 0 or self.state in ('stopped', 'aborted', 'done'):
    ...
    def _startExposure(self, firstExposure=False):
        # Try-except -- CPL        

        if firstExposure or not self.instDoesSequence:
            try:
    ...
    def startSequence(self):
        self._startExposure(firstExposure=True)
