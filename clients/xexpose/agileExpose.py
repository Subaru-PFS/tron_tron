...
class AgileExposureActor(InstExposure.InstExposure):
    def __init__(self, **argv):
        InstExposure.InstExposure.__init__(self, 'agileExpose', debug=1)
        
        self.instName = 'agile'

        self.helpText = ("agileExpose COMMAND [ARGS]",
                         "   COMMAND is one of:",
                         "     help         - you got it!",
    ...
    def _parse(self, cmd):
        ...
        if self.debug >= 0:
            CPL.log("AgileExposureHandler", "new command: %s" % (cmd.raw_cmd))
        ...
        if command == 'setPath':
            ...
            seq = ExpSequence(self, cmd, self.instName, command, path, cnt,
                              debug=1,
                              **seqArgv)
