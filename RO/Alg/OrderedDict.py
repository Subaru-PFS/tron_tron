...
class OrderedDict (dict):
        def __init__(self, ordDict=None):
                """Optionally copies an existing OrderedDict. (You can also copy an ordinary
                dictionary, but the keys will be in undefined order).
                """
                if ordDict == None:
                        dict.__init__(self)
                        self.__keyList = []
                        ...
                        # return a copy
                        dict.__init__(self, ordDict)
                        self.__keyList = ordDict.keys()
        ...
        def popitem(self, i=-1):
                """Removes the ith item from the dictionary (the last item if i is omitted)
        ...
        def update(self, aDict):
                """Adds all items from dictionary aDict to self (in order if aDict is an ordered dictionary).
                """
        ...
        def checkIntegrity(self):
                """Performs an internal consistency check and raises an AssertionError if anything is wrong.
